...
import { DofusSpell, DofusSpellLevel } from '../../../../ts/dofusModels';
...
export class SpellLevel {
	...
	public get summonEffects() {
		let effs = this.spellLevel.effects.filter(e => this.hasSummon(e));
		// console.log("summonEffects: " + asd);
		return effs;
	}
	public hasSummon(e: any) {
		if(this.db.checkFeature("unity")) {
			return db.isSummonEffect(e) && this.db.data.jsonSummons[e.diceNum] && ((e.m_flags & 1) > 0);
		}
	...
	public get hasCondition() {
		return this.spellLevel.statesCriterion && this.spellLevel.statesCriterion != 'null';
	}
    public getConditionsString(spell: DofusSpellLevel) {
        let str = "";
